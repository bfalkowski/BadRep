...
class GitOperations:
    ...
    def get_branch_info(self, branch_name: str) -> BranchInfo:
        ...
        if exists:
            ...
            result = self._run_git_command(["rev-list", "--count", f"{self.config.base_branch}..{branch_name}"])
            if result.returncode == 0:
                ahead_count = int(result.stdout.strip())
            
            result = self._run_git_command(["rev-list", "--count", f"{branch_name}..{self.config.base_branch}"])
            if result.returncode == 0:
...
class GitHubIntegration:
    ...
    def create_pull_request(self, title: str, body: str, head_branch: str, 
                           base_branch: str = "main") -> PullRequestInfo:
    ...
    def get_pull_request(self, pr_id: str) -> PullRequestInfo:
        ...
        command = ["gh", "pr", "view", pr_id, "--json", "id,title,body,state,url,createdAt,updatedAt,mergeable"]
...
class GitLabIntegration:
    ...
    def create_merge_request(self, title: str, description: str, source_branch: str,
