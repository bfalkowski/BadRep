...
from core.pr_workflow import PRWorkflowConfig, PRWorkflowManager, WorkflowResult
...
class TestPRWorkflowManager:
    ...
    def test_execute_workflow_success(self, mock_injection_engine, mock_git_ops):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = workflow_manager.execute_workflow("java", "test_template", "src/Test.java", 42)
    ...
    def test_cleanup_workflow_success(self, mock_injection_engine, mock_git_ops):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mock_git_instance.delete_branch.assert_called_once_with(
                "bug-injection/test-branch", False
            )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mock_results = [
                ...
                MagicMock(
                    success=True,
                    metadata={
                        "files_modified": ["file1.java", "file2.java"],
                        "template_id": "template2",
                    },
                ),
                MagicMock(success=False, metadata={}),
            ]
