...
from core.bug_templates import BugInjection, BugLocation, BugTemplate, BugTemplateManager
    ...
    def export_session_summary(self, session_id: str, output_file: Path):
        ...
        summary = {
            ...
            "statistics": {
                ...
                "success_rate": (
                    session.successful_injections / session.total_injections
                    if session.total_injections > 0
                    else 0
                ),
            },
        }
...
class BugInjectionEngine:
    ...
    def inject_bug(
        ...
        location = BugLocation(file_path=file_path, line_number=line_number)
        ...
        # Log to ground truth
        self.ground_truth_logger.log_injection(self.current_session, injection, result, template)

        return result

    def inject_random_bugs(
        self,
        language: str,
        count: int = 5,
    ...
    def get_available_templates(
        self, language: str, category: Optional[str] = None, difficulty: Optional[str] = None
    ) -> List[BugTemplate]:
