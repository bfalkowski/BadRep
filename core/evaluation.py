...
class EvaluationEngine:
    ...
    def evaluate_review(
        ...
        metrics = self._calculate_metrics(
            len(review_findings), len(ground_truth_entries), len(matches)
        )

        # Identify unmatched items
        unmatched_findings = [f for f in review_findings if f.id not in matched_findings]

        unmatched_ground_truth = [
            gt for gt in ground_truth_entries if gt.id not in matched_ground_truth
        ]
    ...
    def _exact_overlap_match(
        self, finding: ReviewFinding, ground_truth: GroundTruthEntry
    ) -> Optional[MatchResult]:
        """Exact overlap matching based on file and line."""
        if (
            finding.file_path == ground_truth.file_path
            and finding.line_number == ground_truth.line_number
        ):
            return MatchResult(
                finding=finding,
                ...
                if total_keywords:
                    ...
                    if similarity_score > 0.3:  # Threshold for semantic match
                        return MatchResult(
                            ...
                            * 0.8,  # Reduce confidence for semantic matches
    ...
    def _calculate_metrics(
        self, total_findings: int, total_ground_truth: int, true_positives: int
    ) -> EvaluationMetrics:
